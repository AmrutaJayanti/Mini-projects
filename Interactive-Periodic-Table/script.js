document.addEventListener('DOMContentLoaded', () => {
    ...
    const speakButton = document.querySelector('.overlay-btn');

    if (!modal || !overlayElement || !overlayAtomicMass || !overlayAtomicNumber || !overlayBlock || !overlayClass || !overlayState || !speakButton) {
    ...
    function speak() {
        const elementName = overlayElement.textContent;
        if (elementName) {
            const utterance = new SpeechSynthesisUtterance(elementName);
            window.speechSynthesis.speak(utterance);
        } else {
            console.error('No element name to speak.');
        }
    }
